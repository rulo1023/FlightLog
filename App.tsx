...
function EditScreen({ initial, onSave, onDelete, onBack, busy }: {
  ...
  useEffect(() => {
    ...
    if (!canLookup(number)) { setSuggestions([]); setLookupState('idle'); return; }
    if (initial && number === initial.flight_number && localDateKey(draft.flightDate) === initial.flight_date && (initial.departure_airport_code || initial.arrival_airport_code)) {
    ...
    const timer = setTimeout(() => {
      void lookupFlight(number, draft.flightDate).then((results) => {
        if (cancelled) return;
  ...
  }, [draft.flightNumber, draft.flightDate, initial]);
