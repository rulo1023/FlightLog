...
async function testFlighteraDirect() {
  const url = 'https://www.flightera.net/en/flight/VY2616';
  ...
  try {
    ...
    console.log(
      'DIRECT FLIGHTERA LENGTH:',
      text.length,
    );

    console.log(
      'DIRECT FLIGHTERA HAS VY2616:',
      text.includes('VY2616'),
    );
  } catch (error) {
    console.error(
      'DIRECT FLIGHTERA ERROR:',
      error,
    );
  }
}
export async function lookupFlight(flightNumber: string, date: Date): Promise<LookupSuggestion[]> {
  await testFlighteraDirect();
  if (!supabase) return [];
...
async function requestLookup(flightNumber: string, flightDate: string): Promise<LookupSuggestion[]> {
  ...
  if (error) {
    ...
    console.error('EDGE FUNCTION STATUS:', context?.status);

    if (context) {
      try {
        const body = await context.clone().text();
        console.error('EDGE FUNCTION BODY:', body);
      } catch (bodyError) {
        console.error('NO SE PUDO LEER EL BODY:', bodyError);
      }
    }
...
export function applySuggestion(draft: FlightDraft, suggestion: LookupSuggestion): FlightDraft {
  ...
  const source = `Flightera:${suggestion.kind}`;
